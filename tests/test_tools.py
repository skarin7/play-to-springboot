...
class TestState(unittest.TestCase):
    ...
    def test_journal_shrink_is_recorded_as_an_anomaly_and_still_replays(self):
        # A journal shorter than its last recorded offset is indistinguishable
        # from tampering, but the manager must not stall on it -- fold_journal
        # still recovers by replaying from the top. What changes is that the
        # shrink is no longer silent: it lands in journal_anomalies for gate.py
        # to escalate to QA.
        with tempfile.TemporaryDirectory() as d:
            journal = Path(d) / "model-dev.ndjson"
            journal.write_text(
            ...
            s = state.merge_status({})
            self.assertEqual(state.fold_journal(s, journal, "model"), 3)
            self.assertEqual(s["layers"]["model"]["files_migrated"], 6)
            self.assertEqual(s["journal_anomalies"], [])

            # Truncated (or a fresh run reusing the filename) to one line.
            journal.write_text(
                json.dumps({"action": "migrated", "count": 5}) + "\n", encoding="utf-8"
            )
            self.assertEqual(state.fold_journal(s, journal, "model"), 1)
            self.assertEqual(s["layers"]["model"]["files_migrated"], 11)  # replay recovered

            self.assertEqual(len(s["journal_anomalies"]), 1)
            anomaly = s["journal_anomalies"][0]
            self.assertEqual(anomaly["journal"], "model-dev.ndjson")
            self.assertEqual(anomaly["expected_at_least"], 3)
            self.assertEqual(anomaly["found"], 1)
...
class TestReport(unittest.TestCase):
    ...
    def test_main_writes_file_and_prints_path(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            proc = subprocess.run(
                [sys.executable, str(TOOLS_DIR / "report.py"),
...
class TestPermissionHook(unittest.TestCase):
    ...
    HOOK = Path(__file__).resolve().parents[1] / "hooks" / "allow_migration_tools.py"
...
class TestGapRedaction(unittest.TestCase):
    ...
    def test_no_network_code_in_the_tool(self):
        ...
        source = (TOOLS_DIR / "gap_report.py").read_text(
            encoding="utf-8"
        )
